...
STRINGS: Dict[str, Dict[str, str]] = {
    "zh": {
        ...
        "nav_acceptance": "Acceptance · 验收摘要",
        ...
        "audit_layers_note": "所有检查层默认启用；视觉质量（visual_quality）为可选指标，可在下方执行层级中开启。",
        ...
        "audit_tier_header": "⚙️ 执行层级",
        "audit_tier_help": "选择审计执行层级。visual_quality 需要逐帧解码，耗时较长，默认跳过。",
        "audit_tier_fast": "🚀 Fast Audit（默认）",
        "audit_tier_fast_desc": "所有指标，跳过 visual_quality",
        "audit_tier_video_quality": "🎬 Video Quality",
        "audit_tier_video_quality_desc": "全部指标，含 visual_quality",
        "audit_tier_no_video": "🚫 No Video",
        "audit_tier_no_video_desc": "跳过所有视频相关指标（9 项）",
        "audit_tier_video_only": "📹 Video Only",
        "audit_tier_video_only_desc": "仅运行视频相关指标",
        "audit_tier_full": "🔬 Full Audit",
        "audit_tier_full_desc": "全部指标，含 visual_quality",
        "audit_tier_result_label": "当前执行层级",
        "audit_tier_vq_skipped": "⚡ visual_quality 已跳过（Fast Audit 模式）",
        "audit_tier_vq_included": "✅ visual_quality 已执行",

        # ---- page 3: health overview ----
        "health_title": "💚 数据集健康概览",
        "health_caption": "数据集整体质量评估 · 不展示算法细节，只展示结论",
        "health_tier_label": "执行层级",
        "health_tier_note_skipped": "⚡ 本次审计未包含 visual_quality（Fast Audit 模式）。如需启用，请在 Audit 页面切换执行层级后重新运行。",
        "health_tier_note_included": "✅ 本次审计包含 visual_quality 分析。",
        ...
        # ---- page 8: acceptance summary (REQ-5, v0.8.0) ----
        "acc_title": "📋 验收摘要",
        "acc_caption": "面向数据验收方的一页式证据：判定分布、数据集相对基线、时长离群、未检查维度。RDA 只测量并呈现，不作接受/拒收判定。",
        "acc_identity": "数据集信息",
        "acc_episodes": "Episodes",
        "acc_frames": "总帧数",
        "acc_fps": "FPS",
        "acc_robot": "机型",
        "acc_path": "路径",
        "acc_verdict": "判定分布",
        "acc_verdict_pass": "PASS",
        "acc_verdict_review": "REVIEW",
        "acc_verdict_exclude": "EXCLUDE",
        "acc_pass_rate": "通过率",
        "acc_quality": "质量概览",
        "acc_dhi": "DHI",
        "acc_grade": "等级",
        "acc_readiness": "训练就绪度",
        "acc_baseline": "数据集基线（P10 / P50 / P90）",
        "acc_baseline_caption": "基线相对本数据集计算，不套用其他机型的绝对阈值；可用于跨批次对比。",
        "acc_metric": "指标",
        "acc_p10": "P10",
        "acc_p50": "P50（中位）",
        "acc_p90": "P90",
        ...
        "acc_max": "最大",
        "acc_episodes_n": "样本数",
        "acc_unavailable": "该指标在所有 episode 上均不可用，未计算基线。",
        "acc_outliers": "时长离群（Tukey IQR fence）",
        "acc_outliers_caption": "依据 [SLE] score_lerobot_episodes：fence = [Q1 − 1.5·IQR, Q3 + 1.5·IQR]，在本数据集内部计算。",
        "acc_fence": "判定区间：[{lo} s, {hi} s]（Q1={q1} s，Q3={q3} s，IQR={iqr} s）",
        "acc_outlier_count": "检出离群 {n} 条 / 共 {total} 条",
        "acc_outlier_none": "未检出时长离群。",
        "acc_outlier_na": "时长指标在所有 episode 上均不可用，未执行离群检测。",
        "acc_degenerate": "所有 episode 时长完全相同（IQR = 0），fence 退化为单点。这表示「没有离散度可供检测」，不等于「检查过且没问题」。",
        "acc_col_episode": "Episode",
        "acc_col_duration": "时长（s）",
        "acc_col_direction": "方向",
        "acc_dir_long": "偏长",
        "acc_dir_short": "偏短",
        "acc_truncated": "仅显示前 {n} 条（检出总数以上方计数为准）。",
        "acc_not_checked": "未检查维度",
        "acc_not_checked_none": "所有已加载维度均已完成检查。",
        "acc_not_checked_hint": "以下依赖缺失导致对应指标未执行——未检测 ≠ 通过。",
        "acc_calibration": "校准层（Tier-1 可移植指标）",
        "acc_calibration_caption": "Tier-1 指标跨平台可比，与全量指标相关性 ρ = 0.960；平台相关指标（velocity_p95、path_length）不进入基线。",
        "acc_download": "⬇️ 下载验收摘要（JSON）",
        "acc_download_name": "rda_acceptance_summary.json",
        "acc_disclaimer": "本页仅呈现测量结果与相对基线，不发出接受/拒收结论。是否验收由数据验收方决定——RDA 测量并呈现，不裁决语义。",
    },
    ...
    "en": {
        ...
        "audit_layers_note": "All layers are enabled by default; visual_quality is opt-in — select an Execution Tier below to enable.",
        ...
        "audit_tier_header": "⚙️ Execution Tier",
        "audit_tier_help": "Choose the audit execution tier. visual_quality requires frame decoding and is slow; skipped by default.",
        "audit_tier_fast": "🚀 Fast Audit (default)",
        "audit_tier_fast_desc": "All metrics except visual_quality",
        "audit_tier_video_quality": "🎬 Video Quality",
        "audit_tier_video_quality_desc": "All metrics, including visual_quality",
        "audit_tier_no_video": "🚫 No Video",
        "audit_tier_no_video_desc": "Skip all video-related metrics (9)",
        "audit_tier_video_only": "📹 Video Only",
        "audit_tier_video_only_desc": "Only video-related metrics",
        "audit_tier_full": "🔬 Full Audit",
        "audit_tier_full_desc": "All metrics, including visual_quality",
        "audit_tier_result_label": "Execution Tier",
        "audit_tier_vq_skipped": "⚡ visual_quality skipped (Fast Audit)",
        "audit_tier_vq_included": "✅ visual_quality executed",
        ...
        "health_tier_label": "Execution Tier",
        "health_tier_note_skipped": "⚡ This audit did not include visual_quality (Fast Audit). Re-run with a different tier on the Audit page to enable.",
        "health_tier_note_included": "✅ This audit includes visual_quality analysis.",
        ...
        # ---- page 8: acceptance summary (REQ-5, v0.8.0) ----
        "acc_title": "📋 Acceptance Summary",
        "acc_caption": "One page of evidence for the data-acceptance party: verdict distribution, dataset-relative baselines, runtime outliers and what was not checked. RDA measures and presents — it does not adjudicate.",
        "acc_identity": "Dataset",
        "acc_episodes": "Episodes",
        "acc_frames": "Total frames",
        "acc_fps": "FPS",
        "acc_robot": "Robot",
        "acc_path": "Path",
        "acc_verdict": "Verdict distribution",
        "acc_verdict_pass": "PASS",
        "acc_verdict_review": "REVIEW",
        "acc_verdict_exclude": "EXCLUDE",
        "acc_pass_rate": "Pass rate",
        "acc_quality": "Quality",
        "acc_dhi": "DHI",
        "acc_grade": "Grade",
        "acc_readiness": "Training readiness",
        "acc_baseline": "Dataset baseline (P10 / P50 / P90)",
        "acc_baseline_caption": "Computed relative to this dataset rather than borrowed from another robot platform; usable for cross-batch comparison.",
        "acc_metric": "Metric",
        "acc_p10": "P10",
        "acc_p50": "P50 (median)",
        "acc_p90": "P90",
        "acc_min": "Min",
        "acc_max": "Max",
        "acc_episodes_n": "Samples",
        "acc_unavailable": "Unavailable on every episode — no baseline computed.",
        "acc_outliers": "Runtime outliers (Tukey IQR fence)",
        "acc_outliers_caption": "Per [SLE] score_lerobot_episodes: fence = [Q1 − 1.5·IQR, Q3 + 1.5·IQR], computed within this dataset.",
        "acc_fence": "Fence: [{lo} s, {hi} s] (Q1={q1} s, Q3={q3} s, IQR={iqr} s)",
        "acc_outlier_count": "{n} outliers out of {total} episodes",
        "acc_outlier_none": "No runtime outliers detected.",
        "acc_outlier_na": "Duration unavailable on every episode — no outlier test performed.",
        "acc_degenerate": "Every episode has an identical duration (IQR = 0), so the fence collapses to a single point. Read this as \"no spread to detect against\", not as \"checked and clean\".",
        "acc_col_episode": "Episode",
        "acc_col_duration": "Duration (s)",
        "acc_col_direction": "Direction",
        "acc_dir_long": "long",
        "acc_dir_short": "short",
        "acc_truncated": "Showing the first {n}; the count above is exact.",
        "acc_not_checked": "Not checked",
        "acc_not_checked_none": "Every loaded dimension was checked.",
        "acc_not_checked_hint": "Missing optional dependencies left these metrics unrun — not measured ≠ pass.",
        "acc_calibration": "Calibration (Tier-1 portable metrics)",
        "acc_calibration_caption": "Tier-1 metrics are cross-platform comparable and correlate at ρ = 0.960 with the full metric set; platform-specific metrics (velocity_p95, path_length) are deliberately excluded from the baseline.",
        "acc_download": "⬇️ Download acceptance summary (JSON)",
        "acc_download_name": "rda_acceptance_summary.json",
        "acc_disclaimer": "This page presents measurements and relative baselines only; it issues no accept/reject decision. Whether to accept the data belongs to the data-acceptance party — RDA measures and presents, it does not adjudicate semantics.",
    },
}
